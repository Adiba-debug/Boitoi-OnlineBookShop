    ...
    async (req, res) => {
        try {
            ...
            const result = await pool.query(
                ...
                (publisher_name, description, logo_url)
                VALUES ($1, $2, $3)
                RETURNING *`,
                [
                    publisher_name.trim(),
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            console.log(err);
            res.status(500).json({
                message: 'Cannot update publisher'
            });
        }
    }
);
...
router.delete(
    ...
    async (req, res) => {
        try {
            ...
            // Category may be assigned to books
            if (err.code === '23503') {
                return res.status(409).json({
